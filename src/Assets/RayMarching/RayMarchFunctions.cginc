...
{
    return float4(pCol[i].rgb, sdPlane4D(p, pNorm[i], pOff[i]));
}
...
{
    p = p - bPos[i]; // Translates the box
    p = mul(p, bRot[i]); // Rotates the box
    return float4(bCol[i].rgb, sdBox4D(p, bScale[i]));
}
...
{
    p = p - sPos[i]; // Translates the sphere
    p = mul(p, sRot[i]); // Rotates the sphere
    return float4(sCol[i].rgb, sdSphere4D(p, sRad[i]));
}
...
{
    p = p - cPos[i]; // Translates the capsule
    p = mul(p, cRot[i]); // Rotates the capsule
    p.y += cHt[i] / 2.0F;
    return float4(cCol[i].rgb, sdVrtCapsule4D(p, cHt[i], cRad[i]));
}
 ...
 * https://www.youtube.com/playlist?list=PL3POsQzaCw53iKEhOYR39h1J9Lvg-m-g
 *------------------------------------------------------------------*/
...
float softShadows(float4 ro, float4 rd, float4 n)
{
    float res = 1.0;
    float ph = 1e20;
    for (float t = shadeMinDist; t < shadeMaxDist;)
    {
        ...
        res = min(res, shadeSmooth * d / max(0.0, t - y));
        ph = h;
        t += h;
    }
